...
public:
    ...
    BezierCurve(const std::vector<float>& controlpoints, unsigned int tex,
        glm::vec4 amb = glm::vec4(1.0, 0.0, 0.0, 1.0f),
        glm::vec4 diff = glm::vec4(1.0, 0.0, 0.0, 1.0f),
        glm::vec4 spec = glm::vec4(0.1f, 0.1f, 0.1f, 0.5f),
        float shiny = 32.0f)
    {
        // Directly use the passed vector instead of copying the array
        this->cntrlPoints = controlpoints;
        this->ambient = amb;
        this->diffuse = diff;
        this->specular = spec;
        this->shininess = shiny;
